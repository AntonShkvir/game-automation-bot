...
class Tapper:
    def __init__(self, tg_client: Client, context: CallbackContext, user_id):
    ...
    async def get_access_token(self, http_client: aiohttp.ClientSession, tg_web_data: dict[str]):
        try:
            ...
            return access_token
        except http_client.exceptions.HTTPError as http_err:
            ...
            # logger.error(f"{self.session_name} | HTTP error occurred: {http_err}")
        except TypeError as type_err:
            ...
            # logger.error(f"{self.session_name} | Type error: {type_err}")
        except Exception as error:
    ...
    async def run(self, proxy: str | None):
        ...
        async with aiohttp.ClientSession(headers=headers, connector=conn) as http_client:
            ...
            while True:
                threads = load(f"sessions/{self.user_id}/threads.json")
                if self.session_name not in threads:
                    break
                try:
                    ...
                    if boss_current_health <= 0:
                        # logger.info(f"{self.session_name} | Setting next boss: {current_boss_level + 1} lvl")
                    ...
                    if active_turbo is False:
                        ...
                        if settings.AUTO_UPGRADE_CHARGE is True and next_charge_level <= settings.MAX_CHARGE_LEVEL:
                            ...
                            if balance > need_balance:
                                ...
                                if status is True:
                                    # await self.context.bot.send_message(chat_id=self.user_id,
                                    #                                     text=f"{self.session_name} | Charge upgraded to {next_charge_level} lvl")
...
async def run_tapper(tg_client: Client, context, proxy: str | None, user_id):

    try:
        await Tapper(tg_client=tg_client, context=context, user_id=user_id).run(proxy=proxy)
    except InvalidProtocol as error:
        ...
